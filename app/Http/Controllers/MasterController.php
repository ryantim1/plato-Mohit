...
class MasterController extends Controller
{
    ...
    public function index()
    {
    	if(!checkRole(getUserGrade(2)))
      {
        ...
        $data['active_class']       = 'master';
        $data['title']              = getPhrase('department');
        $data['layout']             = getLayout();
        // return view('languages.list', $data);

          $view_name = getTheme().'::master.department.list';
    ...
    public function getDatatable()
    {
    	if(!checkRole(getUserGrade(2)))
      {
        ...
        return back();
         ...
         $records = Master::select([ 'department', 'code'])->orderBy('updated_at','desc');
         ...
         ->addColumn('action', function ($records) {
            $link_data = '<div class="dropdown more">
                         ...
                         <ul class="dropdown-menu" aria-labelledby="dLabel">
                             ...
                             <li><a href="'.URL_DEPARTMENT_EDIT.'/'.$records->slug.'"><i class="fa fa-pencil"></i>'.getPhrase("edit").'</a></li>';
                             $temp = '';
        ...
        {
        	return integerValue($records->code);
        })
    ...
    public function create()
    {
     if(!checkRole(getUserGrade(2)))
      ...
      $data['title']              = getPhrase('add_department');
    	$data['layout']             = getLayout();

        // return view('languages.add-edit', $data);

      $view_name = getTheme().'::master.department.add-edit';
        return view($view_name, $data);
    }
    ...
    public function edit($slug)
    {
      if(!checkRole(getUserGrade(2)))
      {
    	...
    	$record = Master::where('slug', $slug)->get()->first();
    ...
    public function update(Request $request, $slug)
    {
      if(!checkRole(getUserGrade(2)))
      {
        ...
        if($isValid = $this->isValidRecord($record))
         ...
         'department'          		=> 'bail|required|max:40|unique:master,department,'.$record->id,
         'code'           		=> 'bail|required|max:20|unique:,code,'.$record->id,
        ...
        */
    	...
    	return redirect(URL_DEPARTMENT_LIST);
    }
    ...
    public function store(Request $request)
    {
      if(!checkRole(getUserGrade(2)))
      {
       ...
       $this->validate($request, [
         'department'          	=> 'bail|required|max:40|unique:master,department',
         'code'           		=> 'bail|required|max:4|unique:master,code',
   ...
   /* public function changeDefaultLanguage($slug)
    {
      ...
      if(!env('DEMO_MODE')) {
      	$record = Master::where('slug', '=', $slug)->first();
      	$zero = 0;
      	if($isValid = $this->isValidRecord($record))
      		return redirect($isValid);
      	Master::where('id','!=' ,$zero)->update(['is_default'=> $zero]);
      	Master::where('slug', '=', $slug)->update(['is_default'=> 1]);
        Master::resetLanguage();
      }
    	flash('success','record_updated_successfully', 'success');
    	return redirect(URL_DEPARTMENT_LIST);	
    }*/
