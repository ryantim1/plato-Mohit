...
class SectionController extends Controller
{
    ...
    public function index()
    {
    	if(!checkRole(getUserGrade(2)))
      {
        ...
        $data['title']              = getPhrase('section');
        $data['layout']             = getLayout();
        // return view('languages.list', $data);

          $view_name = getTheme().'::master.section.list';
    ...
    public function getDatatable()
    {
    	if(!checkRole(getUserGrade(2)))
      {
        ...
        return back();
         ...
         $records = Section::select(['section'])->orderBy('updated_at','desc');
        ...
        ->addColumn('action', function ($records) {
           $link_data = '<div class="dropdown more">
                        ...
                        <ul class="dropdown-menu" aria-labelledby="dLabel">
                            ...
                            <li><a href="'.URL_SECTION_EDIT.'/'.$records->slug.'"><i class="fa fa-pencil"></i>'.getPhrase("edit").'</a></li>';
                            $temp = '';
    ...
    public function create()
    {
     if(!checkRole(getUserGrade(2)))
      ...
      $data['title']              = getPhrase('add_section');
    	$data['layout']             = getLayout();

        // return view('languages.add-edit', $data);

      $view_name = getTheme().'::section.add-edit';
        return view($view_name, $data);
    }
    ...
    public function edit($slug)
    {
      if(!checkRole(getUserGrade(2)))
      {
    	...
    	$record = Section::where('slug', $slug)->get()->first();
    	if($isValid = $this->isValidRecord($record))
    		return redirect($isValid);
    			
    	  $data['record']       		= $record;
    	  $data['active_class']       = 'section';
        $data['title']              = getPhrase('edit_section');
        $data['layout']             = getLayout();
        // return view('languages.add-edit', $data);
          $view_name = getTheme().'::section.add-edit';
        return view($view_name, $data);
    ...
    public function update(Request $request, $slug)
    {
      if(!checkRole(getUserGrade(2)))
      {
        ...
        if($isValid = $this->isValidRecord($record))
         ...
         'section'          		=> 'bail|required|max:40|unique:Section,section,'.$record->id,
            ]);
       ...
       // $record->code					 = $request->code;
    	...
    	return redirect(URL_SECTION_LIST);
    }
    ...
    public function store(Request $request)
    {
      if(!checkRole(getUserGrade(2)))
      {
       ...
       $this->validate($request, [
         'section'          	=> 'bail|required|max:40|unique:Section,section',
         'code'           		=> 'bail|required|max:4|unique:Section,code',
         'section'          	=> 'bail|required|max:40|unique:Section,section',
            ]);
    	$record = new Section();
        $name 					        = $request->section;
    ...
    public function changeDefaultLanguage($slug)
    {
     ...
     /* if(!env('DEMO_MODE')) {
      	$record = Section::where('slug', '=', $slug)->first();
      	$zero = 0;
      	if($isValid = $this->isValidRecord($record))
      		return redirect($isValid);
      	Section::where('id','!=' ,$zero)->update(['is_default'=> $zero]);
      	Section::where('slug', '=', $slug)->update(['is_default'=> 1]);
        Section::resetLanguage();
      }*/
    	flash('success','record_updated_successfully', 'success');
    	return redirect(URL_SECTION_LIST);	
    }
